...
allResults = model(source, device="cpu", retina_masks=True, imgsz=1024, conf=0.6, iou=0.9)
...
for count, result in enumerate(results[0]):
    ...
    cv2.drawContours(bMask[count], [contour], -1, (255, 255, 255), cv2.FILLED)
    cv2.imshow(f'Contour: {count}', bMask[count])
